    ...
    implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent>
{
  ...
  {
    LOGGER.info( "Create learn user handler is invoked {}", apiGatewayReqEvent.toString() );
    ...
    try
    {
      ...
      if (apiGatewayReqEvent.getMultiValueQueryStringParameters() != null
              && !apiGatewayReqEvent.getMultiValueQueryStringParameters().isEmpty()) {
          multiQueryParams.putAll(apiGatewayReqEvent.getMultiValueQueryStringParameters());
      }
      ...
      Map<String, Object> userInfo = lmsIntegrationhandlerService.getService()
          .createUser( bodyPayload, siteId, applicationId, paramMap, multiQueryParams, lmsType );

      String responseJSON = objectMapper.writeValueAsString( userInfo );
      
      String id = null != userInfo.get( ID ) ? userInfo.get( ID ).toString() : "";
      
      LOGGER.info( "Create User Handler : completed successfully" );
      String locationStr = LOCATION_URL.replace( "{url}", Constants.LEARN_CREATE_USER_PATH )
          .replace( "{id}", id );
      return responseEventHandler.prepareSuccess( responseJSON, apiGatewayReqEvent.getHeaders().get(CORRELATION_ID), locationStr )
